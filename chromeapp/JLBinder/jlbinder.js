    ...
    function ($) {
        ...
        */
        ...
        $.fn.binder.methods = {
            ...
            triggerevent: function (obj, eventname, data, callback, issettimeout) {
                if (jlbinderwindowloaded || issettimeout) {
            ...
            initializeorderby: function (t, stn, inputs) {
                ...
                var orderfieldinput;
                var orderbyinput;
                if (t.attr("data-multiorder")) {
                    $("body").append('<input type="hidden" class="' +
                        orderfieldinputid +
                        '" name="' +
                        stn.orderfieldname +
                        '[0]" value="' +
                    ...
                    ",." +
                    orderbyinputid);
                    orderfieldinput = $("." + orderfieldinputid);
                    orderbyinput = $("." + orderbyinputid);
                }else {
                ...
                this.bodyevent("click",
                    ...
                    function (e) {
                        ...
                        if (t.attr("data-multiorder") && shiftkeypressed) {
                          
                            $("." + orderfieldinputid + "[value=" + $(this).attr("data-orderbyfield") + "]").remove();
                            var orderlength = $("." + orderfieldinputid).length;
                            $("body").append('<input type="hidden" class="' +
                                orderfieldinputid +
                                '" name="' +
                                stn.orderfieldname +
                                '[' + orderlength + ']" value="' +
                                $(this).attr("data-orderbyfield") +
                                '" />');
                            $("body").append('<input type="hidden" class="' +
                                orderbyinputid +
                                '" name="' +
                            ...
                            $("." + orderfieldinputid).not(":first").remove();
                            $("." + orderbyinputid).not(":first").remove();
                    ...
                    function (e) {
                        ...
                        if (stn.scrollloader && !t.attr("data-disablescrollevents")) {
                            ...
                            if (stn.scrolldirection && stn.scrolldirection == "up") {
                                ...
                                if (t.children("[data-jlelements]:visible").first().not("[data-jlscrollerhitted]").length > 0) {
                                    var childbottom = t.children("[data-jlelements]:visible").first().offset().top;
                                    ...
                                    if (childbottom > -50 &&
                                        ...
                                        if ($.active == 0) {
                                            t.children("[data-jlelements]:visible").first().attr("data-jlscrollerhitted", true);
                                            t.binder({ currentpage });
                                        }
                                    }
                                }
                                ...
                                if (t.children("[data-jlelements]:visible").last().not("[data-jlscrollerhitted]").length > 0) {
                                    // -t.children(":visible").last().height()

                                    var childtop = t.children("[data-jlelements]:visible").last().offset().top -
                                        scrollbyelement.height();
                                    if (isscrollbyelement) {
                                        childtop = t.children("[data-jlelements]:visible").last().position().top -
                                            scrollbyelement.height();
                                    }
                                    currentpage = parseInt(scrollerpageinput.val());
                                    if (top > childtop && currentpage < parseInt(t.attr("data-jlpagecount"))) {
                                        ...
                                        currentpage = scrollerpageinput.val();
                                            ...
                                            t.children("[data-jlelements]:visible").last().attr("data-jlscrollerhitted", true);
                                            t.binder({ currentpage });
                                        }
                                    }
                                }
            ...
            create: function (t, opt) {
                ...
                if (this.initfn(t, dataattributes) === false) {
                    ...
                    if (!t.attr("data-binderinit")) {
                        ...
                        this.triggerevent(t, "aftercreate");
                    }
            ...
            processrequest: function (t, stn, inputs, opt) {
                ...
                if (typeof stn.json == "string" && stn.json != "") {
                    ...
                    this.triggerevent(t, "beforeprocessrequest", post, false, true);
            ...
            requesthandler: function (t, method, url, post, cachefn, fn, reqi) {
                ...
                if (reqi < 10) {
                    if (!cachefn()) {
                        this.triggerevent(t, "beforerequest", post,false,true);
                        var mt = this;
                        if ($.active <= 1) {
            ...
            success: function (t, stn, result) {
                ...
                if (!stn.nobind && stn.nobind !== true) {
                    ...
                    var nochangeobj = t.find(templatedetailsobj.nochange).not(excludedelements).not("[data-static-free]");
                    ...
                    if (rows && totalrowsperpage != 0) {
                        ...
                        var scrollerpage;
                        if (t.attr(this.data.data_scrollloader)) {
                            scrollerpage = $("#input_scroller_" + stn.tid).val();
                            pagesize = parseInt($(stn.perpage).val());
                        }
                                ...
                                if (rows.hasOwnProperty(rowindex) && (!isArray || isNaN(rowindex) == false)) {
                                    ...
                                    if (rowobject != null) {
                                        ...
                                        this.loopandsetvalues(t,
                                            ...
                                            pagesize,
                                            pagevalue,
                                            isbindingfreshdata, scrollerpage);
                                    }
                                }
                            }
                ...
                //Only supported id
                var childbinders = $("[data-parent='#" + stn.tid + "'][data-json]:not('[data-bindersuccess]')");
                if (childbinders.length > 0) {
                    childbinders.binder();
                }
                if ($.active <= 1) {
                    $('[id]').each(function () {
            ...
            loopandsetvalues: function (t,
                ...
                if (stn.scrollloader) {
                    rowobject.pagesize = pagesize;
                    rowobject.pagenumber = pagevalue = scrollerpage;
                }

                rowobject.jlrownumber = loopindex + 1;
                
                rowobject.jlfullindex = (pagevalue != null && pagevalue != null) ? ((pagevalue * pagesize) + rowobject.jlrownumber) : rowobject.jlrownumber;
                ...
                for (var key in rowobject) {
                    if (rowobject.hasOwnProperty(key)) {
                        ...
                        var htmlelementsselector = "." +
                            ...
                            key +
                            "']";
                        var htmlelements = template.find(htmlelementsselector);
                        var attrelementsselector = "[data-" +
                            key.toLowerCase() +
                            "-inline],[data-inline-" +
                            key.toLowerCase() +
                            "],[data-" +
                            ...
                            key.toLowerCase() +
                            "]";
                        var attrelements = template.find(attrelementsselector);
                        ...
                        var disabledelements = template.find("[data-" +
                            ...
                            key.toLowerCase() +
                            "],[data-disablejlbind]");
                        obj = obj.not(disabledelements).not(excludedelements).not(excludedblocks.find(htmlelementsselector + "," + attrelementsselector));
                        triggerdata = {
                            key: key,
                            value: value
                        };
                        if (rowobject.jlfullindex == 2 && obj.length > 0) {
                         ///   debugger;
                        }
                                        ...
                                        if (value !== "") {
                                            if (tobj.find('option[value="' + value + '"]').length ==
                                                ...
                                                if (optiontextvalue) {
                                                    ...
                                                    tobj.append("<option value=" + value + ">" + text + "</option>");
                                                }
                                                } else {
